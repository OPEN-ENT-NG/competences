...
import {$, _, Collection, http, IModel, Model} from 'entcore';
import {
    Competence,
    CompetenceNote,
    Eleve,
        ...
        for (let i = 0; i < that.eleves.all.length; i++) {
            ...
            if (_comps.length > 0) {
                ...
                for (let j = 0; j < that.competences.all.length; j++) {
                    ...
                    if (_t === undefined) {
                        _results.push(new CompetenceNote({
                            id_competence: _c.id_competence,
                            nom: _c.nom,
                            id_devoir: that.id,
                ...
                for (let j = 0; j < that.competences.all.length; j++) {
                    _results.push(new CompetenceNote({
                        id_competence: that.competences.all[j].id_competence,
