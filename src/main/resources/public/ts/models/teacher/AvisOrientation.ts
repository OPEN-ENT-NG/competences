...
export class AvisOrientation extends DefaultAvis {
    ...
    async syncAvisOrientation () {
        try {
            let {data} = await http.get(this.api.AVIS_ORIENTATION +
                `?id_eleve=${this.id_eleve}&id_periode=${this.id_periode}&id_structure=${this.id_structure}`);
    ...
    async saveAvisOrientation (idAvisClasse) {
        ...
        if (this.id_avis_conseil_bilan !== undefined && this.id_avis_conseil_bilan !== null) {
            try {
                await http.post(this.api.AVIS_ORIENTATION, this.toJSON());
            } catch (e) {
