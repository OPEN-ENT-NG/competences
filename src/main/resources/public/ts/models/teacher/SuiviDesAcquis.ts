...
import {Achievements, IAchievementsPayload, SubjectAchievements} from "../achievements.model";
...
export class SuiviDesAcquis {
    ...
    displaySkillsValidatedPercentage = (): string => {
        return !!this.skillsValidatedPercentage ? `${this.skillsValidatedPercentage}%` : '';
    }
}
...
export class SuivisDesAcquis extends Model {
    ...
    getAchievements = async (group?: Classe): Promise<SuiviDesAcquis[]> => {
        let payload: IAchievementsPayload = {
        ...
        return Promise.all([
            ...
            achievementsService.getSubjectsSkillsValidatedPercentage(this.idEtablissement, this.idEleve, payload)
        ]).then((values: [AxiosResponse, Achievements]) => {
            if (values[0].data.length > 0) {
                let achievementsList: SuiviDesAcquis[] = Mix.castArrayAs(SuiviDesAcquis, values[0].data);
                values[1].subjectsAchievements.forEach((subjectAchievements: SubjectAchievements) => {
                    let achievements: SuiviDesAcquis = achievementsList.find((achievements: SuiviDesAcquis) =>
                        achievements.id_matiere === subjectAchievements.subjectId
                    );

                    if (!!achievements)
                        achievements.skillsValidatedPercentage = subjectAchievements.skillsValidatedPercentage;
                });
