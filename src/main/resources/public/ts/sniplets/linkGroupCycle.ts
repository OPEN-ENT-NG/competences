...
import {_, idiom as lang, http, notify} from 'entcore';
...
export const linkGroupCycle = {
    ...
    controller: {
        init: function () {
            ...
            this.opened = {
                lightboxLinkGroupCycle : false,
                lightboxDataOnDevoirs: false,
            };
        ...
        getClasses: function () {
            http().get(`/viescolaire/classes?idEtablissement=${this.idStructure}`).done(function (res) {
                this.classesGroupes = {
        ...
        save: function (classes) {
            ...
            http().putJson(`competences/link/groupes/cycles`, jsonToSave).done(() => {
                if(linkGroupCycle.that.selectedRadio.id_cycle != 0) {
                    notify.info('evaluation.link.success');
                }
                else {
                    notify.info('evaluation.unlink.success');
                }
                _.forEach(classes, (classe) => {
                    classe.selected = false;
        ...
        getClasseCycle: (id_cycle) => {
            ...
            return (_cycle !== undefined) ? _cycle.libelle : lang.translate("viescolaire.utils.no.cycle");
        },
