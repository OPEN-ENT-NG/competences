...
import fr.openent.competences.model.achievements.Achievements;
...
public class DefaultBilanPerioqueService implements BilanPeriodiqueService {
    ...
    @Override
    public Future<Achievements> getSubjectSkillsValidatedPercentage(String structureId, String studentId,
                                                                    Long periodId, String groupId) {
        Promise<Achievements> promise = Promise.promise();

        Achievements achievements = new Achievements(structureId, studentId);
        ...
        structureOptionsService.isAverageSkills(structureId)
                ...
                .onSuccess(subjectSkillsValidatedPercentage -> {
                    achievements.setSubjectsAchievements(subjectSkillsValidatedPercentage);
        ...
        sql.prepared(request, params,
                SqlResult.validResultHandler(FutureHelper.handler(promise,
                        String.format("[Competences@%s::getSubjectSkillsValidatedPercentageRequest] " +
                                "Fail to retrieve badge types assigners", this.getClass().getSimpleName()))));
    ...
    private String getSubjectSkillsIsValidatedQuery(String structureId, String studentId,
        ...
        return " SELECT cn.id_eleve, d.id_matiere, id_competence, " +
                String.format(" %s(cn.evaluation) > 1 as is_validated ", Boolean.TRUE.equals(isAverageSkills) ? "AVG" : "MAX") +
