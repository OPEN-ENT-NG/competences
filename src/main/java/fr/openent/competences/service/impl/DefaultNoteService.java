...
public class DefaultNoteService extends SqlCrudService implements NoteService {
    ...
    public void getAppreciationMoyFinalePositionnement(String idEleve, String idMatiere, Long idPeriode,
        ...
        Promise<JsonArray> appreciationPromise = Promise.promise();

        getAppreciationMatierePeriode(idEleve, idMatiere, idPeriode, idGroups, event -> {
            formate(appreciationPromise, event);
        });

        Promise<JsonArray> moyenneFinalePromise = Promise.promise();
        getMoyenneFinale(idEleve, idMatiere, idPeriode, idGroups, event -> {
            formate(moyenneFinalePromise, event);
        });

        Promise<JsonArray> positionnementFinalPromise = Promise.promise();
        getPositionnementFinal(idEleve, idMatiere, idPeriode, event -> {
            formate(positionnementFinalPromise, event);
        });

        Future.all(appreciationPromise.future(), moyenneFinalePromise.future(), positionnementFinalPromise.future()).onComplete(event -> {
            if(event.succeeded()){
                JsonArray appreciation = appreciationPromise.future().result();
                JsonArray moyenneFinale = moyenneFinalePromise.future().result();
                JsonArray positionnement = positionnementFinalPromise.future().result();
    ...
    public void getNotesAndMoyFinaleByClasseAndPeriode(List<String> idsEleve, JsonArray idsGroups, Integer idPeriode,
        ...
        String query = "SELECT * FROM " +
                "(SELECT " + this.table + ".id_eleve AS id_eleve_notes, " + Field.DEVOIRS_TABLE + ".id, " +
                Field.DEVOIRS_TABLE + ".id_periode, " + Field.DEVOIRS_TABLE + ".id_matiere, " + DEVOIRS_TABLE + ".owner, " +
                Field.REL_DEVOIRS_GROUPES_TABLE + ".id_groupe, " + REL_DEVOIRS_GROUPES_TABLE + ".type_groupe, " +
                Field.DEVOIRS_TABLE + ".coefficient, " + Field.DEVOIRS_TABLE + ".diviseur, " + Field.DEVOIRS_TABLE + ".ramener_sur, " +
                Field.DEVOIRS_TABLE + ".is_evaluated, " + this.table + ".valeur, " + Field.DEVOIRS_TABLE +".id_sousmatiere, " +
                TYPE_TABLE + ".formative " +
                "FROM " + this.schema + "devoirs LEFT JOIN " + this.resourceTable +
                " ON ( " + Field.DEVOIRS_TABLE + ".id = " + this.table + ".id_devoir AND " +
                this.table + ".id_eleve IN " + Sql.listPrepared(idsEleve) + " ) " +
                "INNER JOIN " + this.schema + REL_DEVOIRS_GROUPES_TABLE +
                " ON (" + DEVOIRS_TABLE + ".id = " + Field.REL_DEVOIRS_GROUPES_TABLE + ".id_devoir AND " +
                Field.REL_DEVOIRS_GROUPES_TABLE + ".id_groupe IN " + Sql.listPrepared(idsGroups.getList()) + " )" +
                String.format("INNER JOIN %s%s ON %s.id_type = %s.id ", this.schema, Field.TYPE_TABLE, DEVOIRS_TABLE, TYPE_TABLE);
        ...
        if(idPeriode != null){
            query += String.format("WHERE %s.id_periode = ? ", DEVOIRS_TABLE);
            values.add(idPeriode);
        }
        if(idEtablissement != null){
            query += String.format("%s %s.id_etablissement = ? ",(idPeriode != null)? "AND" : "WHERE", DEVOIRS_TABLE);
            values.add(idEtablissement);
        }
        query += String.format(" %s %s.coefficient != 0 AND %s.eval_lib_historise IS FALSE AND %s.is_evaluated IS TRUE AND %s.formative IS FALSE ",
                (idPeriode != null || idEtablissement != null) ? "AND" : "WHERE ", DEVOIRS_TABLE, DEVOIRS_TABLE, DEVOIRS_TABLE, TYPE_TABLE );

        query += " ORDER BY " + this.table + ".id_eleve , " + DEVOIRS_TABLE + ".id_matiere ) AS devoirs_notes " +
                "FULL JOIN ( SELECT " + Field.MOYENNE_FINALE_TABLE + ".id_periode AS id_periode_moyf, " +
                Field.MOYENNE_FINALE_TABLE + ".id_eleve AS id_eleve_moyenne_finale, " + Field.MOYENNE_FINALE_TABLE +
                ".moyenne AS moyenne_finale, " + Field.MOYENNE_FINALE_TABLE  + ".id_matiere AS id_matiere_moyf " +
                "FROM " + this.schema + Field.MOYENNE_FINALE_TABLE +
                " WHERE " + Field.MOYENNE_FINALE_TABLE + ".id_eleve IN " + Sql.listPrepared(idsEleve) +
                ((idPeriode != null) ? " AND " + Field.MOYENNE_FINALE_TABLE + ".id_periode = ? )" : ")") + " AS moyf " +
                "ON (devoirs_notes.id_matiere = moyf.id_matiere_moyf AND devoirs_notes.id_eleve_notes = moyf.id_eleve_moyenne_finale " +
                            ...
                            if (idsEleve.size() == 0) {
                                ...
                                Utils.getGroupesClasse(eb, new fr.wseduc.webutils.collections.JsonArray().add(idClasse),
                                        getGroupesClasseHandler(eleves, handler, idsEleve, idPeriode,
                                                idEtablissement, idClasse, typeGroupe, name, mapAllidMatAndidTeachers,
                                                mapIdMatListMoyByEleve, isYear));
                            }
                        }
                    }
                }
            ...
            public void handle(Either<String, JsonArray> responseQuerry) {
                if (responseQuerry.isLeft()) {
                    ...
                    utilsService.getServices(idEtablissement,
                            idsGroups, FutureHelper.handler(servicesPromise));
                    ...
                    utilsService.getMultiTeachers(idEtablissement,
                            idsGroups, idPeriode.intValue(), FutureHelper.handler(multiTeachingPromise));
                    ...
                    Future.all(servicesPromise.future(), multiTeachingPromise.future(), subTopicCoefFuture)
                            .onComplete(event -> {
                                ...
                                getNotesAndMoyFinaleByClasseAndPeriode(idsEleve, idsGroups, idPeriode, idEtablissement,
                                        calculateAndSetResponseStudentsHandler(nameClasse, idClass, handler, servicesJson,
    ...
    private Handler<Either<String, JsonArray>> calculateAndSetResponseStudentsHandler(String nameClasse, String idClass,
                                                                                             Handler<Either<String, JsonObject>> handler,
        ...
        return response -> {
            if (response.isLeft()) {
                handler.handle(new Either.Left<>("error to get scores"));
                log.error(response.left().getValue());
            } else {
                JsonArray respNotesMoysFinales = response.right().getValue();
               if (respNotesMoysFinales.size() == 0 && !isYear) {
                    handler.handle(new Either.Left<>("no score"));
               } else {
                   ...
                   setMaps(idClass, servicesJSON, multiTeachers, mapAllidMatAndidTeachers, services, respNotesMoysFinales, mapIdEleveIdMatMoy, mapIdEleveIdMatListNotes, mapIdEleveIdMatIdSousMatListNotes);
                   ...
                   //3 - calculate average by eleve by mat by sous mat with mapIdEleveIdMatIdSousMatListNotes and set result in mapIdEleveIdMatMoy
                    Boolean stats = false;
                    Boolean annual = false;
                   calculateAveragesSubjectsWithSubsubjects(mapIdEleveIdMatMoy, mapIdEleveIdMatIdSousMatListNotes, stats, annual);

                   //4 - calculate average by eleve by mat with mapIdEleveIdMatListNotes and set result in mapIdEleveIdMatMoy
                   calculateAverageSubjectsWithoutSubsuject(mapIdEleveIdMatMoy, mapIdEleveIdMatListNotes, stats, annual);
                   ...
                   buildResponseStudents(nameClasse, idClass, handler, servicesJSON, mapAllidMatAndidTeachers, eleves, mapIdMatListMoyByEleve, mapIdEleveIdMatMoy);
               }
            }
        };
    }
    ...
    private void buildResponseStudents (String nameClasse, String idClass, Handler<Either<String, JsonObject>> handler,
                                        JsonArray servicesJSON, SortedMap<String, Set<String>> mapAllidMatAndidTeachers,
                                        List<Eleve> eleves, Map<String, List<NoteDevoir>> mapIdMatListMoyByEleve,
                                        Map<String, Map<String, Double>> mapIdEleveIdMatMoy) {
        JsonArray elevesResult = new JsonArray();
        ...
        for (Eleve eleve : eleves) {
            ...
            JsonArray eleveMoyByMat = new JsonArray();

            eleveJsonO.put(Field.ID_ELEVE, eleve.getIdEleve())
                    .put(LASTNAME, eleve.getLastName())
                    .put(FIRSTNAME, eleve.getFirstName())
                    .put(Field.ID_CLASS, idClass)
            ...
            if (mapIdEleveIdMatMoy.containsKey(eleve.getIdEleve())) {
                ...
                for (Map.Entry<String, Set<String>> setEntry : mapAllidMatAndidTeachers.entrySet()) {
                    ...
                    if (mapIdMatMoy.containsKey(idMatOfAllMat)) {
                        eleveMoyByMat.add(new JsonObject()
                                .put(Field.ID_MATIERE, idMatOfAllMat)
                                .put(MOYENNEBYMAT, mapIdMatMoy.get(idMatOfAllMat)));

                        JsonObject service = servicesJSON.stream()
                                .filter(JsonObject .class::isInstance)
                                .map(JsonObject.class::cast)
                                .filter(el -> idMatOfAllMat.equals(((JsonObject) el).getString(Field.ID_MATIERE)))
                                .findFirst().orElse(null);
                        Double subjectCoefficient = 1.0;
                        if (service != null) {
                            subjectCoefficient = service.getDouble(Field.COEFFICIENT);
                        }

                        NoteDevoir noteDevoir = new NoteDevoir(mapIdMatMoy.get(idMatOfAllMat),
                                20.0, false, subjectCoefficient);
                        ...
                        eleveMoyByMat.add(new JsonObject()
                                .put(Field.ID_MATIERE, idMatOfAllMat)
                                .put(MOYENNEBYMAT, Field.NN));
                    }
                }
                eleveJsonO.put(ELEVEMOYBYMAT, eleveMoyByMat);

                if (!Field.NN.equals(utilsService.calculMoyenne(listMoysEleve, false, 20, false)
                        .getValue(Field.MOYENNE))) {
                    Double moyGeneraleEleve = utilsService.calculMoyenne(listMoysEleve, false, 20, false)
                    ...
                    eleveJsonO.put(MOYGENERALEELEVE, Field.NN);
                }
                ...
                for (Map.Entry<String, Set<String>> setEntry : mapAllidMatAndidTeachers.entrySet()) {
                    ...
                    eleveMoyByMat.add(new JsonObject()
                            .put(Field.ID_MATIERE, idMatOfAllMat)
                            .put(Field.MOYENNEBYMAT, Field.NN));
                    eleveJsonO.put(ELEVEMOYBYMAT, eleveMoyByMat);
                    eleveJsonO.put(MOYGENERALEELEVE, Field.NN);
                }
            }
    ...
    private void calculateAverageSubjectsWithoutSubsuject (Map<String, Map<String, Double>> mapStudentIdSubjectIdAverage,
                                                           Map<String, Map<String, List<NoteDevoir>>> mapStudentIdSubjectIdListScores,
                                                           Boolean stats, Boolean annual) {
        mapStudentIdSubjectIdListScores.entrySet().stream()
            .flatMap(studentEntry -> studentEntry.getValue().entrySet().stream()
                    .map(subjectEntry -> new MutableTriple<>(studentEntry.getKey(), subjectEntry.getKey(), subjectEntry.getValue())))
            .filter(triplet ->
                    !Field.NN.equals(utilsService.calculMoyenne(triplet.getRight(), stats, Field.DIVISEUR_NOTE, annual).getValue(Field.MOYENNE)))
            .forEach(triplet -> {
                Double moy = utilsService.calculMoyenne(triplet.getRight(), stats, Field.DIVISEUR_NOTE, annual).getDouble(Field.MOYENNE);
                setMapIdEleveMatMoy(mapStudentIdSubjectIdAverage, moy, triplet.getLeft(), triplet.getMiddle());
            });

    }
    private void calculateAveragesSubjectsWithSubsubjects (Map<String, Map<String, Double>> mapIdEleveIdMatMoy,
                            HashMap<String, HashMap<String, HashMap<Long, List<NoteDevoir>>>> mapIdEleveIdMatIdSousMatListNotes,
                            Boolean stats, Boolean annual) {
        for (Map.Entry<String, HashMap<String, HashMap<Long, List<NoteDevoir>>>> eleveMapEntry : mapIdEleveIdMatIdSousMatListNotes.entrySet()) {
            for (Map.Entry<String, HashMap<Long, List<NoteDevoir>>> matMapEntry : eleveMapEntry.getValue().entrySet()) {
                ...
                for (Map.Entry<Long, List<NoteDevoir>> sousMatMapEntry : matMapEntry.getValue().entrySet()) {
                    ...
                    if (serv != null && serv.getSubtopics() != null && !serv.getSubtopics().isEmpty()) {
                        SubTopic subTopic = serv.getSubtopics().stream()
                                .filter(el ->
    ...
    private void setMaps (String idClass, JsonArray servicesJSON, JsonArray multiTeachers,
                         SortedMap<String, Set<String>> mapAllidMatAndidTeachers, List<Service> services, JsonArray respNotesMoysFinales,
                         Map<String, Map<String, Double>> mapIdEleveIdMatMoy,
                         Map<String, Map<String, List<NoteDevoir>>> mapIdEleveIdMatListNotes,
                         HashMap<String, HashMap<String, HashMap<Long, List<NoteDevoir>>>> mapIdEleveIdMatIdSousMatListNotes) {
        for (int i = 0; i < respNotesMoysFinales.size(); i++) {
            ...
            String idEleveMoyF = respNoteMoyFinale.getString(ID_ELEVE_MOYENNE_FINALE);
            String idMatMoyF = respNoteMoyFinale.getString(ID_MATIERE_MOYF);
            String idEleveNotes = respNoteMoyFinale.getString(ID_ELEVE_NOTES);
            String idMatiere = respNoteMoyFinale.getString(Field.ID_MATIERE);
            ...
            } else if(idEleveMoyF == null && idEleveNotes != null ) {//pas de moyFinale => set mapIdEleveIdMatListNotes
    ...
    public void getMoysEleveByMatByYear(String idEtablissement, JsonArray periodes, Integer typeGroupe, String name,
        ...
        List<Future<JsonObject>> futureListPeriodes = new ArrayList<>();
        for(int i = 0; i < periodes.size(); i++){
            Promise<JsonObject> promisePeriode = Promise.promise();
            futureListPeriodes.add(promisePeriode.future());
            ...
            getMoysEleveByMatByPeriode(periode.getString(Field.ID_CLASSE), periode.getInteger("id_type"), idEtablissement,
                    typeGroupe, name, mapAllidMatAndidTeachers, mapIdMatListMoyByEleveByPeriode, true,
                    new Handler<Either<String, JsonObject>>() {
                        @Override
                        public void handle(Either<String, JsonObject> event) {
                            if(event.isLeft()){
                                promisePeriode.fail(event.left().getValue());
                                handler.handle(new Either.Left<>( event.left().getValue()));
                                log.error(event.left().getValue());
                                ...
                                promisePeriode.complete();
                            }
                        }
                    });
        }

        Future.all(futureListPeriodes).onComplete( event -> {
            if(event.failed()){
                handler.handle(new Either.Left<>( event.cause().getMessage()));
    ...
    public void getMatEvaluatedAndStat(SortedMap<String, Set<String>> mapAllidMatAndidTeachers,
                                       Map<String, List<NoteDevoir>> mapIdMatListMoyByEleve,
                                       Handler<Either<String, JsonObject>> handler) {
        if( mapAllidMatAndidTeachers.isEmpty() && mapIdMatListMoyByEleve.isEmpty()){
            JsonObject subjectsResult = new JsonObject();
            JsonArray subjects = new JsonArray();

            subjectsResult.put("matieres", subjects);
            subjectsResult.put ("nbDeMatieres", subjects.size());
            handler.handle(new Either.Right<>(subjectsResult));

        } else {
            utilsService.getLibelleMatAndTeacher(mapAllidMatAndidTeachers,  event -> {
                if (!event.isRight()) {
                    log.error(event.left().getValue());
                    handler.handle(new Either.Left(event.left() +
                            String.format("[Competences %s:: getMatEvaluatedAndStat] error to getLibelleMatAndTeacher",
                                    this.getClass().getSimpleName())));
                } else {
                    SortedMap<String, JsonObject> mapRespMatTeacher = event.right().getValue();
                    JsonArray matieresResult = new JsonArray();
            ...
            new DefaultDevoirService(eb).getEleveGroups(idClasse, groupsEvent -> {
                if(groupsEvent.isLeft()){
                    ...
                    competenceNoteService.getConversionNoteCompetence(idEtablissement, idClasse, tableauEvent ->
                            formate(tableauDeConversionPromise, tableauEvent));

                    Promise<JsonArray> formatedServicePromise = Promise.promise();
                    utilsService.getServices(idEtablissement, idGroups, event -> {
                        formate(formatedServicePromise, event);
                    });

                    //Récupération des Services
                    Promise<JsonArray> servicesPromise = Promise.promise();
                    utilsService.getServices(idEtablissement,
                            idGroups, FutureHelper.handler(servicesPromise));

                    //Récupération des Multi-teachers
                    Promise<JsonArray> multiTeachingPromise = Promise.promise();
                    utilsService.getMultiTeachers(idEtablissement,
                            idGroups, idPeriode.intValue(), FutureHelper.handler(multiTeachingPromise));
                    ...
                    List<Future<?>> listFuturesFirst = new ArrayList<>(
                            Arrays.asList(studentsClassFuture, tableauDeConversionPromise.future(), formatedServicePromise.future(),
                                    servicesPromise.future(), multiTeachingPromise.future(), subTopicCoefFuture));
                    for (Object idMatiere : idMatieres){
                        // Récupération du nombre de devoirs avec évaluation numérique
                        Promise<JsonObject> nbEvaluatedHomeWorkPromise = Promise.promise();
                        getNbEvaluatedHomeWork(idClasse, idMatiere.toString(), idPeriode, idGroups, event ->
                                formate(nbEvaluatedHomeWorkPromise, event)
                        );
                        listFuturesFirst.add(nbEvaluatedHomeWorkPromise.future());
                    }

                    // Avec les ids des élèves de la classe, récupération des moyennes Finales , des Notes, des Competences Notes
                    // et des Appreciations et des Positionnements finaux
                    Future.all(listFuturesFirst).onComplete(idElevesEvent -> {
//                        try{
                        if(idElevesEvent.succeeded()) {
                            ...
                            Future<Boolean> isAvgSkillFuture = structureOptionsService.isAverageSkills(idEtablissement);
                            List<Future<?>> listFutures = new ArrayList<>(Arrays.asList(bigRequestPromise.future(), compNotesPromise.future(), notesPromise.future()));
                            listFutures.add(isAvgSkillFuture);
                            Future.all(listFutures).onComplete(event -> {
                                try{
                                    if(event.succeeded()) {
                                        // Rajout des moyennes finales
                                        for (int i = NB_FUTURES; i < idMatieres.size() + NB_FUTURES; i++){
                                            ...
                                            for(Object note : notesPromise.future().result()){
                                        ...
                                        for (Object idMatiere : idMatieres){
                                            ...
                                            for(Object note : compNotesPromise.future().result()){
                                            ...
                                            for (Map.Entry<String, JsonArray> entry : notesByEleve.entrySet()) {
                                                ...
                                                if(elevesMapObject.containsKey(idEleve) && idEleve != null) {
                                                    ...
                                                    calculPositionnementAutoByEleveByMatiere(compNotesEleve, resultNotes,
                                                            annual, tableauDeConversionPromise.future().result(),
                                        ...
                                        for (int i = 6; i < idMatieres.size() + 6; i++){
                                            ...
                                            FormateColonneFinaleReleveTotale(bigRequestPromise.future().result(), elevesMapObject,
                                        ...
                                        getMoyenneGeneraleMinMax(elevesMapObject, idPeriode, idMatieres,
                                                servicesPromise.future().result(), annual, resultHandler);
    ...
    private void getStudentClassForExportReleve(String idClasse, Long idPeriode, JsonArray idEleves, Integer typeClasse,
                                                ...
                                                Promise<JsonArray> studentsClassPromise ){
        new DefaultUtilsService(this.eb).studentAvailableForPeriode(idClasse, idPeriode, typeClasse,
                message -> {
                    ...
                    if ("ok".equals(body.getString("status"))) {
                        ...
                        studentsClassPromise.complete(eleves);
                    }
                    else {
                        studentsClassPromise.fail("[getStudentClassForExportReleve] " +
    ...
    private Future <JsonArray> getStudentClassForExportReleve(String classId, Long periodId, JsonArray studentsIds, Integer typeClass,
        ...
        getStudentClassForExportReleve(classId, periodId, studentsIds, typeClass,studentMapObject, promiseStudentsClass);
    ...
    public void exportPDFRelevePeriodique(JsonObject param, final HttpServerRequest request, Vertx vertx,
        ...
        getDatasReleve(param, event -> formate(exportResultPromise, event));
        ...
        Promise<JsonObject> imgStructurePromise = Promise.promise();
        utilsService.getParametersForExport(idStructure, event -> formate(imgStructurePromise, event));

        // Récupération du libellé de la période
        Promise<String> periodeLibellePromise = Promise.promise();
        getLibellePeriode(eb, request, param.getInteger(ID_PERIODE_KEY),  periodeLibelleEvent -> {
            formate(periodeLibellePromise, periodeLibelleEvent );
        });

        Future.all(exportResultPromise.future(), structurePromise.future(), imgStructurePromise.future(), periodeLibellePromise.future())
                .onComplete((event -> {
                    if (event.failed()) {
                        ...
                        JsonObject exportJson = exportResultPromise.future().result();
                        putLibelleAndParamsForExportReleve(exportJson, param);

                        JsonObject imgStructure =  imgStructurePromise.future().result();
                        ...
                        exportJson.put("periodeLibelle", periodeLibellePromise.future().result());
    ...
    public void getDataGraph(final String idEleve, JsonArray groupIds, final String idEtablissement ,
        ...
        isAvgSkillFuture
                .onSuccess(isAvgSkillpromiseResult -> Future.all(compNotePromise.future(), notePromise.future(), subjectPromise.future(), tableauDeConversionPromise.future()).onComplete(event -> {
                    if(event.failed()){
                    ...
                    Future.all(servicesPromise.future(), multiTeachingPromise.future(), subTopicCoefFuture).onComplete(e -> {
                        if (e.failed()) {
                            String message = "[getReleveDataForGraph] " + event.cause().getMessage();
                            log.error(message);
                            handler.handle(new Either.Left<>(message));
                            ...
                            final JsonArray listCompNotes = compNotePromise.future().result();
                            final JsonArray listNotes = notePromise.future().result();
                            ...
                            linkIdSubjectToLibelle(idEleve, idPeriode, getMaxOrAvgByItem(matieresCompNotes, tableauDeConversionPromise.future().result(),isAvgSkillpromiseResult ),
                                    getMaxOrAvgByItem(matieresCompNotesEleve, tableauDeConversionPromise.future().result(),isAvgSkillpromiseResult ), matieresNotes, matieresNotesEleve,
                                    mapMatieresStatClasseAndEleve, idMatieres, subjectPromise.future().result(), handler);
                        }
                    });
                }))
                .onFailure(err ->{
    ...
    public void getDataGraphDomaine(final String idEleve, JsonArray groupIds, final String idEtablissement ,
        ...
        isAvgSkillFuture
                .onSuccess(isAvgSkillpromiseResult -> {
                    Future.all(compNotesPromise.future(), domainesCyclePromise.future(), tableauDeConversionPromise.future()).onComplete(event -> {
                        if (event.succeeded()) {
                            handler.handle(new Either.Right<>(linkCompNoteToLibelle(domainesCyclePromise.future().result(),
                                    compNotesPromise.future().result(), tableauDeConversionPromise.future().result(), idEleve, isAvgSkillpromiseResult)));
                        } else {
    ...
    public void getDetailsReleve(final String idEleve, final String idClasse, final String idMatiere,
        ...
        Promise<JsonArray> listCompNotesPromise = Promise.promise();
        getCompetencesNotesReleve(idEtablissement, idClasse,null, idMatiere, null,
                idEleve,null, true, true, event -> formate(listCompNotesPromise, event));
        detailsFuture.add(listCompNotesPromise.future());

        // Récupération des notes de l'élève
        Promise<JsonArray> listNotePromise = Promise.promise();
        getNoteElevePeriode(null, idEtablissement, new JsonArray().add(idClasse), idMatiere, null,
                event -> formate(listNotePromise, event));
        detailsFuture.add(listNotePromise.future());

        // Récupération des moyennes finales de tous les élèves de la classe
        Promise<JsonArray> moyFinalesElevesPromise = Promise.promise();
        getColonneReleve(null, null, idMatiere,
                new JsonArray().add(idClasse), Field.MOYENNE, Boolean.FALSE, event -> formate(moyFinalesElevesPromise, event));
        detailsFuture.add(moyFinalesElevesPromise.future());
        ...
        Promise<JsonArray> appreciationMatierePeriodePromise = Promise.promise();
        getColonneReleve(new JsonArray().add(idEleve), null, idMatiere, new JsonArray().add(idClasse),
                APPRECIATION_MATIERE_PERIODE, Boolean.FALSE, event -> formate(appreciationMatierePeriodePromise, event));
        detailsFuture.add(appreciationMatierePeriodePromise.future());

        Promise<JsonArray> moyenneFinalesPromise = Promise.promise();
        getColonneReleve(new JsonArray().add(idEleve), null, idMatiere, new JsonArray().add(idClasse),
                Field.MOYENNE, Boolean.FALSE, event -> formate(moyenneFinalesPromise, event));
        detailsFuture.add(moyenneFinalesPromise.future());

        Promise<JsonArray> positionnementPromise = Promise.promise();
        getColonneReleve(new JsonArray().add(idEleve), null, idMatiere, new JsonArray().add(idClasse),
                Field.POSITIONNEMENT, Boolean.FALSE, event -> formate(positionnementPromise, event));
        detailsFuture.add(positionnementPromise.future());
        ...
        competenceNoteService.getConversionNoteCompetence(idEtablissement, idClasse, tableauEvent ->
                formate(tableauDeConversionPromise, tableauEvent));
        detailsFuture.add(tableauDeConversionPromise.future());
        ...
        Future.all(detailsFuture).onComplete(event -> {
            if(event.failed()){
            ...
            result.put("appreciations", appreciationMatierePeriodePromise.future().result());
            result.put("moyennes_finales", moyenneFinalesPromise.future().result());
            result.put("positionnements", positionnementPromise.future().result());

            // Calcul des positionements
            JsonArray listCompNotes = listCompNotesPromise.future().result();
            calculPositionnementAutoByEleveByMatiere(listCompNotes, result,false, tableauDeConversionPromise.future().result(),
                    null ,null, isAvgSkillFuture.result());

            // Calcul des moyennes par période pour la classe
            JsonArray moyFinalesEleves = moyFinalesElevesPromise.future().result();

            HashMap<Long, HashMap<Long, ArrayList<NoteDevoir>>> notesByDevoirByPeriodeClasse =
                    calculMoyennesEleveByPeriode(listNotePromise.future().result(), result, idEleve, idEleves,
                            null , null,services,multiTeachers );
